...
**Acceptance Criteria:**
- [ ] `go.mod` initialized with a sensible module name (e.g., `github.com/dirnei/maggus`)
- [ ] Cobra dependency added
- [ ] `main.go` exists and calls the root command
- [ ] `cmd/root.go` defines the root command with a basic description
- [ ] `maggus --help` prints usage info
...
**Acceptance Criteria:**
- [ ] `cmd/work.go` defines the `work` subcommand under root
- [ ] Accepts an optional positional argument for task count (e.g., `maggus work 10`)
- [ ] Accepts a `--count` / `-c` flag as alternative (e.g., `maggus work --count 10`)
- [ ] Defaults to `5` if no count is provided
- [ ] Positional argument takes precedence over flag if both are given
- [ ] `maggus work --help` prints usage info explaining the count option
- [ ] For now, the command just prints: `Starting work on <N> tasks...`
...
**Acceptance Criteria:**
- [ ] A `parser` package exists (e.g., `internal/parser/`)
- [ ] Parser reads all `.maggus/plan_*.md` files from the current working directory
- [ ] Each `### TASK-xxx: Title` heading is extracted as a task
- [ ] The `**Description:**` text is extracted per task
- [ ] The `**Acceptance Criteria:**` checkboxes are extracted per task
- [ ] Each criterion tracks its checked state (`[x]` = done, `[ ]` = open)
- [ ] A task is considered "complete" when all its criteria are `[x]`
- [ ] Unit tests cover: parsing a valid plan, extracting multiple tasks, detecting complete vs. incomplete tasks
...
**Acceptance Criteria:**
- [ ] A function returns the next incomplete task (first task where any criterion is `[ ]`)
- [ ] If all tasks are complete, it returns nil/empty with an appropriate indication
- [ ] Tasks are ordered by their appearance in the file (TASK-001 before TASK-002, etc.)
- [ ] If multiple plan files exist, tasks from earlier files (by filename sort) come first
- [ ] Unit tests cover: finding next task, all-done scenario, ordering across files
- [ ] Typecheck/lint passes
...
**Acceptance Criteria:**
- [ ] A `prompt` package exists (e.g., `internal/prompt/`)
- [ ] The prompt includes the task title, description, and all acceptance criteria
- [ ] The prompt instructs the agent to work on this specific task only
- [ ] The prompt tells the agent to verify acceptance criteria before finishing
- [ ] Unit test verifies prompt contains all task fields
- [ ] Typecheck/lint passes
...
**Acceptance Criteria:**
- [ ] Maggus invokes `claude` via `os/exec` with the `-p` flag and the built prompt
- [ ] stdout and stderr from claude are streamed to the user's terminal in real-time
- [ ] If `claude` exits with a non-zero code, Maggus logs the error and stops processing further tasks
- [ ] If `claude` is not found on PATH, a clear error message is shown
- [ ] Typecheck/lint passes
...
**Acceptance Criteria:**
- [ ] The `work` command parses plans, finds the next incomplete task, builds the prompt, and invokes Claude Code
- [ ] After each task completes, Maggus moves to the next incomplete task
- [ ] Processing stops after N tasks (user-specified count) or when no incomplete tasks remain
- [ ] Before each task, Maggus prints which task it is starting (e.g., `[1/5] Working on TASK-003: Parse plan files...`)
- [ ] After all tasks are done (or count reached), Maggus prints a summary (e.g., `Completed 3/5 tasks. 2 tasks remaining.`)
- [ ] Typecheck/lint passes

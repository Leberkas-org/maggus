...
**Acceptance Criteria:**
- [ ] Old `display` struct, `newDisplay`, `renderLocked`, and ANSI cursor manipulation code are fully removed (no dead code)
- [ ] `go build ./...` succeeds with no warnings
- [ ] `go vet ./...` passes
- [ ] `go test ./...` passes
- [ ] The `stderrWriter`, `describeToolUse`, and `truncate` helpers are preserved (still needed by the stream parser)

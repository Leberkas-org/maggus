...
**Acceptance Criteria:**
- [ ] In `updateList` (`status_update.go`), when the key is `b` and `m.selectedTask() != nil` and the task has at least one blocked criterion, open the detail overlay and immediately enter criteria mode (same effect as Enter followed by Tab)
- [ ] If the task has no blocked criteria, `b` is a no-op (or shows a brief "no blocked criteria" note)
- [ ] The hint bar (`status_view.go`) shows `b: unblock` when a blocked task is selected in the tree and the detail overlay is not open
- [ ] The existing Enter → Tab path continues to work unchanged

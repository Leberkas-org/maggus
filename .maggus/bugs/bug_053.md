...
**Acceptance Criteria:**
- [ ] In `viewStatus()` (`src/cmd/status_view.go`), remove the `lipgloss.NewStyle().Faint(true).Render(bg)` call — pass `bg` directly to `styles.OverlayCenter` instead of `dimmedBg`
- [ ] The status view background is fully visible (not faint/dimmed) when the F1 popup is open

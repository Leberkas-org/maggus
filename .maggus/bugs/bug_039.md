...
**Acceptance Criteria:**
- [ ] `snapshotForSelectedTask()` checks that the returned snapshot's `TaskID` matches the selected task's ID
- [ ] If `m.snapshot.TaskID != selectedTask.ID`, returns nil (not the wrong snapshot)
- [ ] For parallel mode, `m.workerSnapshots[task.ID]` still takes precedence when available
- [ ] When no matching snapshot exists, the Output tab shows "Waiting for agent output..." (not stale data)
...
**Acceptance Criteria:**
- [ ] In `nullTUIModel.writeSnapshot()` (`daemon_tui.go`), `snap.TaskID` is always set from `m.taskID` — verify this is never empty during active work
- [ ] `IterationStartMsg` handler sets `m.taskID` BEFORE the first `writeSnapshot()` call in the same message cycle
- [ ] If `m.taskID` is empty when `writeSnapshot()` is called, the snapshot includes a field indicating "no active task" (not silently empty)
- [ ] `go vet ./...` and `go test ./...` pass
...
**Acceptance Criteria:**
- [ ] When the daemon starts a new work cycle, it removes completed/failed entries from `state-workers.json` that are older than 5 minutes
- [ ] When a dispatched worker completes (success or failure), it removes its own entry from the workers index OR updates status to done/failed (already partially done)
- [ ] `refreshWorkerSnapshots()` skips entries where the per-worker snapshot file doesn't exist
- [ ] The status TUI never shows ghost workers from previous runs
- [ ] `go vet ./...` and `go test ./...` pass

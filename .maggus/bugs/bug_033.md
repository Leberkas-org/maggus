...
**Acceptance Criteria:**
- [ ] `renderCurrentTaskTab` does NOT apply `.Width().Height()` wrapping — it returns raw content and lets `renderRightPane` handle final sizing
- [ ] `renderRightPane` applies `.MaxHeight(height)` (not `.Height()`) to the combined `tabBar + sep + content` so overflow is clipped rather than pushing the border
- [ ] Alternatively: each tab renderer pre-truncates lines to `width` before returning, preventing lipgloss word-wrap from adding extra lines
- [ ] The outer border frame remains visible on all tabs: Output, Summary, Details, Task Details, Metrics
- [ ] Long acceptance criteria text in Task Details does not break the layout
- [ ] Task descriptions with very long lines do not break the layout
- [ ] No regression in other tab renderers (they all follow the same pattern)

...
**Acceptance Criteria:**
- [ ] `buildApprovedPlans()` in `src/cmd/run_loop.go` no longer calls `approval.Prune()`
- [ ] The daemon only calls `approval.Load()` (read-only) to determine which plans are approved
- [ ] Pruning remains in the TUI path only (`reloadPlans()` in `status_model.go` and `pruneStaleApprovals` calls in `menu_model.go` / `app_model.go`)
- [ ] Creating a new feature file while the daemon is processing an approved feature does NOT change the approved feature's approval state
- [ ] The daemon continues working on the approved feature undisturbed after new files are created
- [ ] No regression in approval toggle from the TUI (`a` key)

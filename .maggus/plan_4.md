...
**Acceptance Criteria:**
- [ ] After a `maggus vision` session ends, usage is extracted from the Claude session file
- [ ] A `usage.Record` is appended to `.maggus/usage_vision.jsonl` with the same fields as TASK-002
- [ ] If session detection or extraction fails, a warning is printed to stderr and the command exits successfully
- [ ] Typecheck/lint passes
- [ ] Unit tests are written and successful
...
**Acceptance Criteria:**
- [ ] After a `maggus architecture` session ends, usage is extracted from the Claude session file
- [ ] A `usage.Record` is appended to `.maggus/usage_architecture.jsonl` with the same fields as TASK-002
- [ ] If session detection or extraction fails, a warning is printed to stderr and the command exits successfully
- [ ] Typecheck/lint passes
- [ ] Unit tests are written and successful

...
func (m statusModel) viewStatusSplit() string {
	...
	rightPane := m.renderStatusRightPane(rightW, innerH)
...
// renderStatusRightPane renders the right pane task content for the selected plan.
// This is a transitional implementation; subsequent tasks (TASK-001-004 through
// TASK-001-007) will replace it with the full tabbed view.
func (m statusModel) renderStatusRightPane(width, height int) string {
	if width <= 0 {
	...
	if m.planCursor < len(visible) {
		...
		sb.WriteString("\n ")
		sb.WriteString(buildProgressBar(done, total))
		summary := fmt.Sprintf("  %d/%d tasks · %d pending · %d blocked",
			done, total, pending, blocked)
		sb.WriteString(statusDimStyle.Render(summary))
		...
		for taskIdx := m.ScrollOffset; taskIdx < end; taskIdx++ {
			...
			line := fmt.Sprintf("%s%s  %s: %s", prefix, icon, t.ID, t.Title)
			line = leftPaneTruncate(line, width-2)
			sb.WriteString("\n")
			sb.WriteString(style.Render(line))
		}
		if len(m.Tasks) > visibleLines {
		...
		sb.WriteString("\n")
		sb.WriteString(statusDimStyle.Render("  No feature selected"))
	}

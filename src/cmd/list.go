...
// collectTasksFromFiles collects workable, ignored, and incomplete tasks from parsed files.
func collectTasksFromFiles(files []string) (workable, ignored, incomplete []parser.Task, err error) {
	for _, f := range files {
		tasks, parseErr := parser.ParseFile(f)
		if parseErr != nil {
			return nil, nil, nil, fmt.Errorf("parse %s: %w", f, parseErr)
		}
	...
	return workable, ignored, incomplete, nil
}

var listCmd = &cobra.Command{
	Use:   "list [N]",
	Short: "Preview the next N upcoming workable tasks",
	Long:  `Reads all feature and bug files and lists the next N workable (incomplete, not blocked) tasks. Bug tasks appear before feature tasks.`,
...
func runList(cmd *cobra.Command, dir string, plain, all bool, count int) error {
	...
	// Parse bug files first (bugs take priority over features).
	bugFiles, err := parser.GlobBugFiles(dir, false)
	if err != nil {
		return fmt.Errorf("glob bugs: %w", err)
	}
	bugWorkable, bugIgnored, bugIncomplete, err := collectTasksFromFiles(bugFiles)
	if err != nil {
		return err
	}
	...
	featureWorkable, featureIgnored, featureIncomplete, err := collectTasksFromFiles(featureFiles)
	if err != nil {
		return err
	}

	// Merge: bugs first, then features.
	workable := append(bugWorkable, featureWorkable...)
	ignored := append(bugIgnored, featureIgnored...)
	incomplete := append(bugIncomplete, featureIncomplete...)

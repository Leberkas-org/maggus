...
func Create(repoDir, worktreeDir, branch string) error {
	cmd := gitutil.Command("worktree", "add", worktreeDir, "-b", branch)
	cmd.Dir = repoDir
...
func Remove(repoDir, worktreeDir string) error {
	cmd := gitutil.Command("worktree", "remove", worktreeDir, "--force")
	cmd.Dir = repoDir
	if out, err := cmd.CombinedOutput(); err != nil {
...
func List(repoDir string) ([]string, error) {
	cmd := gitutil.Command("worktree", "list", "--porcelain")
	cmd.Dir = repoDir
	out, err := cmd.Output()
	if err != nil {
...
func Prune(repoDir string) error {
	cmd := gitutil.Command("worktree", "prune")
	cmd.Dir = repoDir
	if out, err := cmd.CombinedOutput(); err != nil {
...
func DeleteBranch(repoDir, branch string) error {
	cmd := gitutil.Command("branch", "-D", branch)
	cmd.Dir = repoDir
	if out, err := cmd.CombinedOutput(); err != nil {
